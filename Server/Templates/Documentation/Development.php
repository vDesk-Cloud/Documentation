...
<p>
    This section complains about coding conventions and requirements on the source code.
</p>
<hr>
...
<p>
    vDesk strictly uses the "PascalCase"-notation except for the names of JavaScript <a target="_blank" href="https://developer.mozilla.org/en-US/docs/Web/API/CustomEvent">CustomEvents</a> and a small amount of facades to built-in objects in the client (this may be refactored in the future?).<br>
    The purpose of this notation is to visually separate library code from runtime code, because vDesk provides many object-oriented interfaces for general tasks like database access or file manipulation.
</p>
...
<p>
    ...
    That means source code have to  provide as much type-safety as possible.
</p>
<h5>PHP</h5>
<p>
...
<p>
    ...
    However, if you <u>optionally</u> want to use value type-checking, you can use the <code class="Inline">Ensure.<?= Code::Function("Parameter") ?>()</code> and <code class="Inline">Ensure.<?= Code::Function("Property") ?>()</code> methods.
</p>
<hr>
...
<p>Parameters must be named according their purpose. Basically, code should provide a "beautiful API"<?= Code::Delimiter ?> that means parameter names must not consist of abbreviations.</p>
<pre><code><?= Conventions::NotRecommended ?>
...
<p>
    ...
    <br>
    For a JavaScript based code example, visit the "<a href="<?= Functions::URL("Documentation", "Page", "Tutorials", "Tutorial", "ClassicalInheritance#Interfaces") ?>">Classical inheritance and interfaces in JavaScript</a>"-tutorial.
</p>
<pre><code><?= Conventions::NotRecommended ?>
...
<h5>JavaScript</h5>
<p>
    Use <code class="Inline"><?= Code::Class("Array") ?>.<?= Code::Field("prototype") ?>.<a target="_blank" href="https://developer.mozilla.org/de/docs/Web/JavaScript/Reference/Global_Objects/Array/forEach"><?= Code::Function("forEach") ?></a></code> in combination with lambda predicates over "for" and "while".
</p>
<pre><code><?= Conventions::NotRecommended ?>
...
<h3 id="UI">UI</h3>
<p>This section complains about visual guidelines</p>
<p>
...
<p>Until the system is capable of handling and <a href="<?= Functions::URL("vDesk", "Page", "Roadmap#Colors") ?>">coloring SVG-images</a>, icons have to meet the following conditions:</p>
<ul>
...
<p>
    This section complains about working with SQL-database servers.
</p>
<hr>
<h4 id="SQL">SQL</h4>
<p>
    Although the "Structured Query Language" already provides an abstract way of communicating with database servers, there are still exist some inconsistencies between certain RDBMS.<br>
    ...
    So it still may happen that for example database "A" only supports the "MODIFY"-kewyord, while database "B" only supports the "ALTER"-keyword while database "C" supports both, and so on..
</p>
<p>
    To address the common differences between several RDBMs, vDesk's dataprovider ships with an "Expresseion"-library, that allows the developer to "express" the opration you want to perform against the database server in a parametrized monade that translates the action into a query that is compatible to the current configured database system.
...
<p>
    ...
    you may pass any plain SQL-strings via the <code class="Inline"><?= Code::Class("DataProvider") ?>::<?= Code::Function("Execute") ?>()</code>-method directly to the underlying driver (like <a target="_blank" href="https://www.php.net/manual/de/mysqli.real-query.php">mysqli::real_query()</a>, <a target="_blank" href="https://www.php.net/manual/de/function.pg-query.php">pg_query()</a> or <a target="_blank" href="https://www.php.net/manual/de/pdo.query.php">PDO::query()</a>.<br>
...
<p>
    ...
    That means, that any current model is capable of lazy loading values for single properties
</p>
<ol>
...
<p>
    If your Model references a dependency Model or database value, then consider using mapped getters/setters.
</p>
