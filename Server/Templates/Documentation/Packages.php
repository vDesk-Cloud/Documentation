...
<p>
    ...
    <br>
    For further information about creating packages and setups, visit the <a href="<?= Functions::URL("Documentation", "Page", "Tutorials", "Tutorial", "CustomReleases") ?>">Custom releases</a>-tutorial.
</p>
<h3>Overview</h3>
<ul class="Topics">
...
<p>
    A usual package consists of a package manifest class file placed in the <code class="Inline"><?= Code::Console("/Server/Lib/vDesk/Packages") ?></code> directory while any client- or server-side resources are stored in certain directories.
</p>
...
<ul>
    ...
    <li>Declaring a public "Version"-constant holding a version number that is compatible with PHP's built in <a target="_blank" href="https://www.php.net/manual/de/function.version-compare.php">\version_compare()</a>-function</li>
...
<p>
    ...
    While installation, the package gets passed to ever "installer"-module, that has to care for itself for a compatible package type, in setup context; this applies to every bundled package.
</p>
<p>
...
<p>
    ...
    To be recognized as a potential installer, the module must implement the <code class="Inline">\vDesk\Package\<?= Code::Class("IModule") ?></code>-interface<br> and provide an "Install"-method that processes the package.<br>
    To keep complexity of the package system as simple as possible, the installer has to check each passed package for a compatible package type and must resolve required constants before processing.
...
<?= Code::ClassDeclaration ?> <?= Code::Class("CustomInstaller") ?> <?= Code::Extends ?> \vDesk\Modules\<?= Code::Class("Modules") ?> {
    ...
    <?= Code::Public ?> <?= Code::Static ?> <?= Code::Function ?> <?= Code::Function("Install") ?>(\vDesk\Package <?= Code::Variable("\$Package") ?>, \Phar <?= Code::Variable("\$Phar") ?>, string <?= Code::Variable("\$Path") ?>): <?= Code::Void ?> {
...
<aside class="Note">
    ...
    <p>
        An alternative may enforcing "Files", "Locale", etc... to be static methods returning arrays, but for general information like name or license, we still have to stick to base classes.<br>
...
<p>
    A usual update consists of an update manifest class file placed in the <code class="Inline"><?= Code::Console("/Server/Lib/vDesk/Packages") ?></code> directory<br>
    as well as an updated version of the target package manifest class file placed in the <code class="Inline"><?= Code::Console("/Server/Lib/vDesk/Updates") ?></code> directory<br>
...
<ul>
    ...
    <li>Declaring a public "Package"-constant holding an array with the target package and required minimum version</li>
    <li>Declaring a public "Version"-constant holding a version number that represents the upgraded version of the target package</li>
    ...
    <li>(Optionally)Declaring a public "Files"-constant holding the resources to add, update or delete of the update</li>
    <li>Implementing a public static "Install"-method</li>
</ul>
<pre><code><?= Code\Language::PHP ?>
<?= Code::PHP ?>
...
<?= Code::ClassDeclaration ?> <?= Code::Class("CustomUpdate") ?> <?= Code::Extends ?> \vDesk\<?= Code::Class("Update") ?> {
    ...
    <?= Code::Public ?> <?= Code::Constant ?> <?= Code::Const("Package") ?> = [<?= Code::String("\"ExamplePackage\"") ?> => <?= Code::String("\"0.9.1\"") ?>]<?= Code::Delimiter ?>
...
<p>
    ...
    As difference to single packages, the package manifest class-files are stored in their target directory, instead of the archive root.
</p>
